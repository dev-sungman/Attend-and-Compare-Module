...
def ACM(x, blockname, groups=32):
    ...
    mu = tf.expand_dims(mu, axis=1)
    P = Conv2D(c//2, 1, padding='same', groups=groups, name=blockname+'_P1')(mu)
    P = relu(P)
    P = Conv2D(c, 1, padding = 'same', groups=groups, name=blockname+'_P2')(P)
    P = sigmoid(P)

    x_mu = x - mu
    k = Conv2D(c, 1, padding='same', groups=groups, name=blockname+'_K')(x_mu)
    q = Conv2D(c, 1, padding='same', groups=groups, name=blockname+'_Q')(x_mu)
    k = softmax(k)
    q = softmax(q)
    k = x_mu * k
    q = x_mu * q
